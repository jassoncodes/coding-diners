    ...
    class Ticket : HelperRpa
    {
        ...
        public List<tikets> GetSMTickets(string smXlFileFullPath)
        {
            ...
            }catch(Exception ex)
            {
                ...
                LogFile log = new LogFile(DateTime.Now, errorMessage, "ERROR");
        ...
        public bool crear(tikets ticket)
        {
            if(ticket.operacion == "CREAR")
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LogFile log = new LogFile(DateTime.Now, "Creando el ticket: " + ex.Message.ToString(), "Error");
                    LogFile.WriteToLog(log);
        ...
        public bool modificar(tikets ticket)
        {
            if (ticket.operacion == "MODIFICAR")
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LogFile log = new LogFile(DateTime.Now, "Modificando el ticket: " + ex.Message.ToString(), "Error");
                    LogFile.WriteToLog(log);
                    Process.GetCurrentProcess().Kill();
                    Environment.Exit(0);
                    return false;
